...
function finalizeExplanation(explanation: DecisionExplanation): DecisionExplanation {
...
export function decideIntakeV2(result: WizardResult): IntakeDecision {
  ...
  const refinedDecision: IntakeDecision = applySignalRefinements(baseline, result);
...
): decision is IntakeDecision & { decisionModelVersion: typeof DECISION_MODEL_VERSION_V2 } {
