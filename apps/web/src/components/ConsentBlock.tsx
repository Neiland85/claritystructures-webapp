...
const CONSENT_COPY: Record<IntakeTone, { text: string; warning?: string }> = {
  basic: {
...
export default function ConsentBlock({ tone, checked, onChange }: Props) {
  ...
  return (
    <div className="space-y-2 border border-neutral-700 p-4 rounded">
      ...
      {copy.warning && <p className="text-xs text-red-400">{copy.warning}</p>}
    </div>
  );
}
