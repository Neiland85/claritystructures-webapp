...
function deriveEvidenceLevel(result: WizardResult): EvidenceLevel {
  const normalizedSources = result.evidenceSources.map(
...
function deriveExposureState(result: WizardResult): ExposureState {
  const actions = result.actionsTaken.map(
...
/* -----------------------------------------------------------
   Refinements (V2 contextual adjustments)
------------------------------------------------------------ */
...
export function mapWizardToSignals(
  result: WizardResult
): IntakeSignals {
  const riskLevel = refineRiskLevel(deriveRiskLevel(result), result);
  const evidenceLevel = refineEvidenceLevel(
    deriveEvidenceLevel(result),
    result
  );
  const exposureState = refineExposureState(
