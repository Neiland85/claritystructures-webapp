...
import type { SubmitIntakePayload } from "@claritystructures/domain";
...
function validatePayload(payload: unknown): string[] {
  ...
  if (!isString(candidate.clientProfile))
    errors.push("clientProfile must be a string");
  ...
  if (!isFiniteNumber(candidate.devices))
    errors.push("devices must be a finite number");
  if (!isStringArray(candidate.actionsTaken))
    errors.push("actionsTaken must be a string[]");
  if (!isStringArray(candidate.evidenceSources))
    errors.push("evidenceSources must be a string[]");
...
test("submitting valid data to /api/intake/submit returns 202", async () => {
  ...
  const fixture = intakeFunnelFixtures.find(
    (entry) => entry.name === "baseline",
  );
...
test("submitted intake is persisted with decision and explanation structure", async () => {
  const db = new InMemoryIntakeDb();
  const fixture = intakeFunnelFixtures.find(
    (entry) => entry.name === "v2_refinement",
  );
